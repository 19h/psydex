...
impl PsyDex {
    ...
    #[inline]
    fn load_drug_interaction (client: &client::HTTP, drug_name: &str, drug: &mut Drug) {
        let ts_drug_uri = ((&*TS_URL_DRUG_PREFIX).to_string() + drug_name).to_string();

        println!("Loading {:?}", ts_drug_uri);

        let res = client.get(&ts_drug_uri);
    ...
    fn load_drug_interactions (&mut self) {
        ...
        for (drug_name, mut drug) in &mut self.drugs {
            self::PsyDex::load_drug_interaction(&self.client, &drug_name, &mut drug);
        }
    }
}
